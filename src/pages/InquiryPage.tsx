  ...
  return (
    <div className="max-w-6xl mx-auto py-10 px-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-6">
        <div>
          <h2 className="text-lg font-semibold text-foreground">문의사항</h2>
          <p className="text-sm text-muted-foreground mt-1">궁금한 점을 자유롭게 문의해 주세요.</p>
        </div>
        <button
      ...
      {/* Auth code notice */}
      <div className="flex items-start gap-3 p-4 border border-border bg-secondary mb-8">
              ...
              <tbody className="text-sm">
                {inquiries.map((item) => (
                  <tr key={item.id} className="border-b border-border last:border-0 hover:bg-secondary/50 transition-colors">
                    <td className="px-5 py-3 font-medium text-foreground whitespace-nowrap">{item.title}</td>
                    <td className="px-5 py-3 text-muted-foreground max-w-xs truncate">{item.content}</td>
                    <td className="px-5 py-3 text-foreground">{item.author}</td>
